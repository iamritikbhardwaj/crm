      ...
      const bookings = data.map((item) => (
          {
            ...
            apayment: <div className={`${item.orderValue === item.payment ? "text-green-500" : "text-red-500"}`}>{item.payment + " USD"}</div>,
            status: <button className={`${item.status === "CANCELLED" && "bg-red-500"} ${item.status === "CONFIRMED" && "bg-green-500"} ${item.status === "IN-PROGRESS" && "bg-blue-500"} text-white p-1 m-1 rounded-full`}>{item.arrivalDate < new Date() ? "IN-PROGRESS" : item.status}</button>,
